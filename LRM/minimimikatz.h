...
#define SAM_ACCOUNT_PATH L"\\REGISTRY\\MACHINE\\SAM\\SAM\\Domains\\Account\\Users"
...
typedef struct _SYSKEY {
	UCHAR data[SYSKEY_LENGTH];
} SYSKEY, *PSYSKEY;

typedef struct _SAMKEY {
	UCHAR data[SAM_KEY_DATA_KEY_LENGTH];
} SAMKEY, *PSAMKEY;

typedef struct _NTLM {
	UCHAR data[LM_NTLM_HASH_LENGTH];
} NTLM, *PNTLM;
...
typedef struct _SAM_KEY_DATA {
	DWORD Revision;
	DWORD Length;
	CHAR Salt[SAM_KEY_DATA_SALT_LENGTH];
	CHAR Key[SAM_KEY_DATA_KEY_LENGTH];
...
typedef struct _SAM_KEY_DATA_AES {
	...
	DWORD DataLen;
	CHAR Salt[SAM_KEY_DATA_SALT_LENGTH];
	CHAR data[ANYSIZE_ARRAY]; // Data, then Check
